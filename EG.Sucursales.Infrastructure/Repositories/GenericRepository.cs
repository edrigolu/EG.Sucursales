...
namespace EG.Sucursales.Infrastructure.Repositories
{
    public abstract class GenericRepository<T> : IGenericRepository<T> where T : class
    {
        ...
        public virtual async Task<IEnumerable<T>> ListarAsync()
        {
            ...
            return await connection.QueryAsync<T>(sp, commandType: CommandType.StoredProcedure);
        }
        ...
        public virtual async Task<int> CrearAsync(T entidad)
        {
        ...
        public virtual async Task<bool> ActualizarAsync(T entidad)
        {
            string sp = $"eg_{_prefix}_actualizar_{_entidad}";
            using var connection = _context.CreateConnection();
            var parameters = MapToParameters(entidad);
            var filas = await connection.ExecuteAsync(sp, parameters, commandType: CommandType.StoredProcedure);
            return filas > 0;
        }
